...
SS = 4;%state size
N = 5;%population size
gen = 100;  %generation size
v = [rand(N,1)*(80-40) + 40,rand(N,1)*(25-10) + 10,rand(N,1)*(13-8) + 8,rand(N,1)*(2.5-0.5) + 0.5];
...
if max(F2)-min(F2) == 0
    ...
    MF2 = (F2-(min(F2))) ./ (max(F2)-(min(F2)));%scale fitness 0 to 1  
end
...
%pause
for i = N+1:2*N
    [stiff,weight]  = RUN_ABAQUS_BELLOW_VJ(R(i,:));
    F1(i) = stiff;
    F2(i) = weight;
end
